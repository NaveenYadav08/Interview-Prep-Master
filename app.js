...
const email = "greatguy494@gmail.com";
const username = "NAVEEN";
const password = "123456";
// createAdminCumUser(email, username, password);
...
const dbURI =
  "mongodb+srv://Arnav:Arnav2000@cluster0-0eldl.mongodb.net/Interview?retryWrites=true&w=majority";
//mongoose connect
mongoose
  .connect(dbURI, {
...
const appsetup = (database) => {
  ...
  app.get("/topics/:t_name", requireAuth, (req, res) => {
    ...
    database.db
      .collection("topics")
      .findOne({ name: topic.toString() })
      //then get all questions linked to it
      .then((topic) => {
        let questions = database.db
  ...
  app.get("/companys/:c_name", requireAuth, (req, res) => {
    ...
    database.db
      .collection("companies")
      .findOne({ name: company.toString() })
      //then get all experiences linked to it
      .then((company) => {
        let experiences = database.db
